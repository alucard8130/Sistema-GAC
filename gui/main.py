...
class PantallaPrincipal():  
    def __init__(self):
        ...
        self.pp.actionCarga_Clientes.setVisible(False)
        self.pp.actionCarga_Locales.setVisible(False)
        self.pp.actionCarga_Areas_Comunes.setVisible(False)
        self.pp.actionCarga_Contratos.setVisible(False)
        self.pp.actionCarga_Saldos_Iniciales.setVisible(False)
        self.pp.actionRegCuentas.setVisible(False)
        self.pp.btnCambiarUsuario.setVisible(False)
        ...
        self.pp.btnCambiarUsuario.clicked.connect(self.reiniciar_sistema)
                ...
                try:
                    for row in data:
                        ...
                        if objData.registrar(carga_clientes):
                            m.setText("Clientes cargados con exito")
                        else:
 ...
 ##############################################LOG IN###################################################       
    def validar_acceso(self):
        ...
        if  self.pp.txtUsuario.text()=="":
            ...
            if res :
                ...
                self.pp.btnIngresar.setVisible(True)
                self.pp.btnCambiarUsuario.setVisible(True)
                self.pp.statusBar.showMessage("Bienvenido" " "+ nom_user,0)
                self.pp.lblName_User.setText(nom_user)
                if nom_user == "administrador":
                    self.pp.actionCarga_Clientes.setVisible(True)
                    self.pp.actionCarga_Locales.setVisible(True)
                    self.pp.actionCarga_Areas_Comunes.setVisible(True)
                    self.pp.actionCarga_Contratos.setVisible(True)
                    self.pp.actionCarga_Saldos_Iniciales.setVisible(True)
                    self.pp.actionRegCuentas.setVisible(True)
                else:
                    pass       
            else:
    ...
    def reg_new_user(self):
        ...
        #self.pp.cmbEmpresa.setVisible(False)
...
################################## PAGOS---COBRANZA---LC --######################################        
    ...
    def set_cmb_factura_fpcm(self):
        search=FacturasData()
        data=search.lista_facturas(self.ppcm.cmblocal.currentText(),self.ppcm.cmbCliente.currentText())
        for item in data:
        ...
        if self.fpcm.cmbFactura.currentIndex()==0:
            ...
            if  objData._registrar(info=regpago):
                ...
                reg_bd_c=Reg_Cartera(
                    ...
                    clienteFact=self.ppcm.cmbCliente.currentText(),
  ...
  #############################PANTALLA PRINCIPAL GESTOR AREAS COMUNES#############################################
   ...
   ################################## PAGOS---COBRANZA--AC --######################################        
    ...
    def set_cmb_factura_fpac(self):
        search=FacturasData()
        data=search.lista_facturas(self.ppac.cmbAreaC.currentText(),self.ppac.cmbCliente.currentText())
        for item in data:
    ...
    def registrar_cobranzaAC(self):
        ...
        if self.fpac.txtAreaC.text()=="":
            ...
            if  objData._registrar(info=regpago):
                ...
                clienteFact=self.ppac.cmbCliente.currentText(),
    ...
    def registrar_facturacionAC(self):
        ...
        if self.ffac.txtAreaC.text()=="":
            ...
            regpago=Reg_Factura(   
                ...
                cliente=self.ppac.cmbCliente.currentText(),
            ...
            if  objData._registrar(info=regpago):
                ...
                if self.ffac.cmbTipoFact.currentIndex() == 2 or self.ffac.cmbTipoFact.currentIndex() == 3:
                    importeAdeudo = -1 * float(self.ffac.txtImporte.text())
                else:
                    importeAdeudo = float(self.ffac.txtImporte.text())
...
##################################BUSCAR Y EDITAR CONTRATOS#############################################    
    ...
    def guardar_edicion(self):
        m=QMessageBox()
        m.setIcon(QMessageBox.Icon.Information)
        m.setWindowTitle("Desea editar el contrato ?")
...
#####################################TABLA DE INFORMACION CUOTAS MANTTO############################################        
    ...
    def mostrar_informacionCM(self):
        if self.ppcm.cmblocal.currentIndex()==0 :
            ...
            data=search.buscar_info_ubicacion(self.ppcm.cmblocal.currentText(),self.ppcm.cmbCliente.currentText())
            fila=0
            saldo=0
            if data==[]:
                m=QMessageBox()
...
########################################CARGA MASIVA DE CUOTAS###########################################################
    ...
    def seleccion_tipo_cartera(self):
        if self.ffm.cmbTcartera.currentIndex()==0 :
            ...
            m.exec()
        elif self.ffm.fechaPeriodo.date().toString("yyyy-MM-dd") < current_datetime.strftime("%Y-%m-%d"):
            m=QMessageBox()
    ...
    def registro_masivo_cuotas_locales(self):
        ...
        m.setText("¿¿Se generaran las cuotas de locales comerciales??")
    ...
    def registro_masivo_cuotas_areas(self):
        ...
        m.setIcon(QMessageBox.Icon.Information)
        m.setWindowTitle("Carga Masiva.")
        m.setText("¿¿Se generaran las cuotas de Areas Comunes??")
        m.setStandardButtons(QMessageBox.StandardButton.Ok | QMessageBox.StandardButton.Cancel)
        res=m.exec()
